...
**请不要将此项目用于生产**
## 怎么用
首先建议使用 GNU/Linux 操作系统（macOS 也行），并且安装**最新版** Python 以及 `tqdm` 和 `pillow` 库。  
**`make` 段：**`cd` 到此目录，然后执行 `make/make.sh`。将生成在根目录的 `rawya.fnt` 拿 [Bits'n'Picas](https://github.com/kreativekorp/bitsnpicas) 打开。  
**`tools` 段：** 暂时废弃。  
